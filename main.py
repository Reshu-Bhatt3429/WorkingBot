...
║                                                                    ║
║  Strategy (reverse-engineered from @Hcrystallash):               ║
║    1. Every 5-min market: buy the CHEAP side as a hedge           ║
║    2. After 60s, load up on whichever direction BTC/ETH moved    ║
║    3. Average in if direction keeps holding                       ║
║    4. Exit early if 70%+ profit, otherwise hold to resolution    ║
...
class HedgeBot:
    ...
    def run(self):
        ...
        logger.info(f"\n{'═'*60}")
        logger.info(f"  POLYMARKET HEDGE BOT | {mode}")
    ...
    def _tick(self):
        ...
        if now < self._cooldown_until:
            ...
            if int(now) % 30 == 0:
                logger.info(f"⏸️  Cooldown active — {remaining_cd}s remaining")
            self._maybe_display()
            return
        ...
        for asset in ASSETS:
            ...
            # Update hedge engine (may place orders)
    ...
    def _resolve_market(self, asset: str):
        ...
        if pnl > 0:
            ...
            if self.consecutive_losses >= MAX_CONSECUTIVE_LOSSES:
                ...
                logger.warning(
                    f"⚠️  {self.consecutive_losses} consecutive losing markets — "
    ...
    def _refresh_balance(self):
        ...
        if balance > 0:
            ...
            logger.info(f"💵 Bankroll updated: ${balance:.2f} USDC → Kelly sizing recalibrated")
